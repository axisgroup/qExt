...
describe("qExt Scripts", function() {
	...
	before("Copying template directory", function() {
		...
		return new Promise(resolve => {
			fs.copy(templateDirectory, testDirectory).then(() => {
				resolve()
			})
		})
	})
  ...
  ================================ */
	describe("vanilla build", function() {
		before("Running qext-scripts", function() {
			return new Promise(resolve => {
				...
				fs.readJson(`${testDirectory}/qext.config.json`)
					...
					.then(config =>
						fs.writeJson(`${testDirectory}/qext.config.json`, config)
					)
					/* run qext-scripts */
					.then(() => {
						exec(
							"node ../../bin/qext-scripts",
							{
								cwd: testDirectory
							},
							function() {
		...
		it("should create dist folder with the source folder copied in", function(done) {
			...
			Promise.all([templateFiles, distFiles]).then(
				([templateFiles, distFiles]) => {
	...
	describe("compile build", () => {
		before("Running qext-scripts", () => {
			return new Promise(resolve => {
				...
				vanillaDone$.pipe(filter(status => status)).subscribe(() => {
		...
		it("should create dist folder with built extension project inside", done => {
			fs.stat(`${testDirectory}/dist/${extensionName}`).then(() => {
				done()
			})
		})
		...
		it("should have a built extension javascript file inside", done => {
			fs.stat(
				`${testDirectory}/dist/${extensionName}/${extensionName}.js`
			).then(() => {
				done()
			})
		})
		...
		it("should have a qext file inside", done => {
			fs.stat(
				`${testDirectory}/dist/${extensionName}/${extensionName}.qext`
			).then(() => {
				done()
			})
		})
		...
		it("should have a static directory inside", done => {
			fs.stat(`${testDirectory}/dist/${extensionName}/static`).then(() => {
				done()
			})
		})
