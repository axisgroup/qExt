...
describe("01. Copy source directory", function() {
	...
	it("should copy source directory into output", done => {
		execQextScripts(testExtensionDir, (error, stdout, stderr) => {
			...
			// const sourceCopy = srcDirectory.then(srcDierctory => {
			// 	const distDirectory = fs.readdir(`${testExtensionDir}/dist/${extensionName}`)
			// 	return distDirectory.then(distDirectory => {
			// 		return distDirectory.should.deep.equal(srcDirectory)
			// 	})
			// })
			// sourceCopy.then(console.log)
			// const sourceCopy = Promise.all([srcDirectory, distDirectory]).then(([srcDirectory, distDirectory]) => {
			// 	return distDirectory.should.deep.equal(srcDirectory)
			// })
